...
public class KargoEkle extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        getContentPane().add(jLabel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(374, 101, -1, -1));
        ...
        jLabel24.setIcon(new javax.swing.ImageIcon(getClass().getResource("/KargoEntityBean/ucakbult.jpg"))); // NOI18N
        jLabel24.setText("jLabel24");
        getContentPane().add(jLabel24, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 930, 620));

        setSize(new java.awt.Dimension(939, 651));
    ...
    private void btnKaydetActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKaydetActionPerformed
        ...
        k.setAlantc(txtalıcıtc.getText());
        k.setGönderentc(txtGönderenTc.getText());
       
      

        Alankisi ak = new Alankisi();
        ak.setTc(txtalıcıtc.getText());
        ak.setAd(txtAlıcıAd.getText());
        ak.setSoyad(txtAlıcıSoyad.getText());
        ak.setTel(txtAlıcıTel.getText());
        ak.setAdres(txtAlıcıAdres.getText());
        
        
       
   

        Gonderenkisi gk = new Gonderenkisi();
        gk.setTc(txtGönderenTc.getText());
        gk.setAd(txtGönderenAdı.getText());
        gk.setSoyad(txtGönderenSoyad.getText());
        gk.setAdres(txtGönderenAdres.getText());
        gk.setTel(txtGönderenTel.getText());
...
em.getTransaction().commit();
      ...
      //   em.close();
       ...
       // Query q= em.createQuery(" SELECT KARGOID FROM KargoEkle WHERE alanTc=? AND gönderenTC=?");
        Query q=em.createQuery("SELECT k FROM Kargolar k WHERE k.alantc=:atc AND k.gönderentc=:gtc");
        
        q.setParameter( "atc", txtalıcıtc.getText());
        q.setParameter("gtc", txtGönderenTc.getText());
        
        List <Kargolar> kargoList=q.getResultList();
        for (Kargolar kargoList1 : kargoList) {
            id = kargoList1.getKargoid();
        }
        em.close();
        emf.close();
